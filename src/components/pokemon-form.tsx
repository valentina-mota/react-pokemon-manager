...
const PokemonForm: FunctionComponent<Props> = ({pokemon, isEditForm}) => {
  ...
  const types: string[] = [
    ...
    'Poison', 'Fairy', 'Flying', 'Fighting', 'Psychic'
  ];
  ...
  const validateForm = () => {
    ...
    if(!/^[a-zA-Zàéè ]{3,25}$/.test(form.name.value)) {
      const errorMsg: string = "The name of the pokemon is required (1-25).";
    ...
    if(!/^[0-9]{1,3}$/.test(form.hp.value)) {
      const errorMsg: string = "A Pokémon's HP goes from 0 to 999.";
    ...
    if(!/^[0-9]{1,2}$/.test(form.cp.value)) {
      const errorMsg: string = "A Pokémon's damage ranges from 0 to 99.";
